...
local uipallet = {
  ...
  Themes = {
    ...
    ['Snowy Sky'] = {
      { Color3.fromRGB(1, 171, 179), Color3.fromRGB(234, 234, 234), Color3.fromRGB(18, 232, 232) },
      6,
      10,
    },
...
local getcustomassets = {
  ['Lunar Vape/Assets/Rise/Slice.png'] = 'rbxasset://79307254401837',
  ['Lunar Vape/Assets/Rise/Blur.png'] = 'rbxasset://14898786664',
  ...
  writefile(
    ...
    httpService:JSONEncode {
      ...
      faces = {
        {
          style = 'normal',
          assetId = getcustomasset 'Lunar Vape/Assets/Rise/SF-Pro-Rounded-Light.otf',
          name = 'Light',
          weight = 300,
        },
        {
          style = 'normal',
          assetId = getcustomasset 'Lunar Vape/Assets/Rise/SF-Pro-Rounded-Regular.otf',
          name = 'Regular',
          weight = 400,
        },
        {
          style = 'normal',
          assetId = getcustomasset 'Lunar Vape/Assets/Rise/SF-Pro-Rounded-Medium.otf',
          name = 'Medium',
          weight = 500,
        },
        {
          style = 'normal',
          assetId = getcustomasset 'Lunar Vape/Assets/Rise/Icon-1.ttf',
          name = 'Icon1',
          weight = 600,
        },
        {
          style = 'normal',
          assetId = getcustomasset 'Lunar Vape/Assets/Rise/Icon-3.ttf',
          name = 'Icon3',
          weight = 800,
        },
      },
...
components = {
  ...
  ColorSlider = function(optionsettings, children, api)
    ...
    function optionapi:SetValue(h, s, v, o)
      ...
      if self.Rainbow then
        ...
        valuebutton.Text = math.round(text.R * 255)
          .. ', '
          .. math.round(text.G * 255)
          .. ', '
          .. math.round(text.B * 255)
      end
  ...
  Dropdown = function(optionsettings, children, api)
    ...
    dropdown.MouseButton1Click:Connect(function()
      optionapi:SetValue(
        optionsettings.List[(table.find(optionsettings.List, optionapi.Value) % #optionsettings.List) + 1],
        true
      )
    end)
  ...
  Toggle = function(optionsettings, children, api)
    ...
    knobholder.Position = UDim2.fromOffset(
      getfontsize(optionsettings.Name, 18, uipallet.Font).X + (optionsettings.Darker and 20 or 0) + 22,
      10
    )
  ...
  TwoSlider = function(optionsettings, children, api)
    ...
    fill.Size = UDim2.fromScale(
      math.clamp(math.clamp(optionapi.ValueMax / optionsettings.Max, 0, 1), 0, 1) - fill.Position.X.Scale,
      1
    )
    fill.BackgroundColor3 = color.Dark(uipallet.MainColor, 0.5)
    fill.Parent = bkg
...
function mainapi:CreateCategory(categorysettings)
  ...
  if categorysettings.Name == 'Search' then
    createHighlight(
      UDim2.fromOffset(buttonsize.X + 49, 30),
      UDim2.fromOffset(20, button.AbsolutePosition.Y - mainframe.AbsolutePosition.Y)
    )
  ...
  function categoryapi:CreateModule(modulesettings)
    ...
    mwindowlist:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
      ...
      if modulechildren.Visible then
        ...
        tween:Tween(
          modulebutton,
          TweenInfo.new(math.min(height * 3, 450) / 1000, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out),
          {
  ...
  button.MouseButton1Click:Connect(function()
    if not guiTween or guiTween.PlaybackState ~= Enum.PlaybackState.Playing then
      ...
      createHighlight(
        UDim2.fromOffset(buttonsize.X + 49, 30),
        UDim2.fromOffset(20, (button.AbsolutePosition.Y - mainframe.AbsolutePosition.Y) / scale.Scale)
      )
      title.TextColor3 = uipallet.Text
      icon.TextColor3 = uipallet.Text
  ...
  if isfile('Lunar Vape/Profiles/' .. game.GameId .. ' GUI Settings.txt') then
    ...
    if not skipgui then
      ...
      for i, v in guidata.Categories do
        ...
        if not object or object.Type ~= 'Overlay' and object.Type ~= 'CategoryTheme' then
          continue
        end
        if v.Theme then
...
function mainapi:Save(newprofile)
  ...
  for i, v in self.Categories do
    if v.Type ~= 'Overlay' and v.Type ~= 'CategoryTheme' then
      continue
    end
  ...
  if interface.Button.Enabled then
    ...
    for i, v in mainapi.Modules do
      if v.Enabled or table.find(found, i) then
        if interfaceshow.Value == 'Exclude render' and v.Category == 'Render' then
          continue
        end
        if interfaceshow.Value == 'Only bound' and #v.Bind <= 0 then
          continue
        end
        if i == 'RiseInterface' then
          continue
        end
        local holder = Instance.new 'Frame'
        holder.Name = i
        ...
        holderbackground.Size =
          UDim2.fromOffset(getfontsize(moduletext, 21, uipallet.Font).X + (v.ExtraText and 10 or 8), 22)
        ...
        holdertext.Text = i
          .. (
            v.ExtraText
              and interfacesuffix.Enabled
              and " <font color='rgb(200, 200, 200)'>" .. v.ExtraText() .. '</font>'
            or ''
          )
        if interfacelowercase.Enabled then
        ...
        if interfacebar.Enabled then
          ...
          holderline.Image = getcustomasset 'Lunar Vape/Assets/Rise/Slice.png'
...
function mainapi:UpdateGUI(hue, sat, val, default)
  ...
  if not default and self.GUIColor.Rainbow then
    return
  end
  if interface.Button.Enabled then
    watermarkgradient.Color = ColorSequence.new {
    ...
    for _, v in InterfaceLabels do
      if interfacearraycolor.Value == 'Static' then
        ...
        v.Text.TextColor3 =
          self:RiseColor(interfacearraycolor.Value == 'Fade' and v.Text.AbsolutePosition / 2 or Vector2.new())
      end
      if v.Color then
  ...
  if targetinfoobj.Button.Enabled then
    targetinfogradient.Color = ColorSequence.new {
      ...
      ColorSequenceKeypoint.new(
        1,
        color.Dark(self:RiseColor((targetinfobkg.AbsolutePosition + Vector2.new(0, 100)) / 2), 0.5)
      ),
    }
    targetinfohealthgradient.Color = ColorSequence.new {
      ColorSequenceKeypoint.new(0, uipallet.MainColor),
  ...
  if not inputService:GetFocusedTextBox() and inputObj.KeyCode ~= Enum.KeyCode.Unknown then
    ...
    if checkKeybinds(mainapi.HeldKeybinds, mainapi.Keybind, inputObj.KeyCode.Name) then
      ...
      guiTween = tweenService:Create(
        ...
        TweenInfo.new(
          0.3,
          mainapi.Visible and Enum.EasingStyle.Exponential or Enum.EasingStyle.Linear,
