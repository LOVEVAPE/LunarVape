...
components = {
  ...
  Slider = function(optionsettings, children, api)
    ...
    function optionapi:Color(hue, sat, val, rainbowcheck)
      ...
      knob.BackgroundColor3 = mainapi.GUIColor.Rainbow and color.Light(uipallet.Main, 0.14)
  ...
  Toggle = function(optionsettings, children, api)
    ...
    function optionapi:Color(hue, sat, val, rainbowcheck)
      ...
      knob.BackgroundColor3 = mainapi.GUIColor.Rainbow and uipallet.Main
        or mainapi:TextColor(hue, sat, val, uipallet.Main)
    end

    function optionapi:Toggle()
      self.Enabled = not self.Enabled
      local rainbowcheck = mainapi.GUIColor.Rainbow and mainapi.RainbowMode.Value ~= 'Retro'
      knobholder.BackgroundColor3 = self.Enabled
  ...
  TwoSlider = function(optionsettings, children, api)
    ...
    fill.Size = UDim2.fromScale(
      math.clamp(math.clamp(optionapi.ValueMax / optionsettings.Max, 0, 1), 0, 0.99) - fill.Position.X.Scale,
      1
    )
    fill.BackgroundColor3 = Color3.fromHSV(mainapi.GUIColor.Hue, mainapi.GUIColor.Sat, mainapi.GUIColor.Value)
    fill.Parent = bkg
    ...
    function optionapi:SetValue(max, value)
      ...
      tween:Tween(fill, TweenInfo.new(0.1), {
        ...
        Size = UDim2.fromScale(
          math.clamp(math.clamp(math.clamp(self.ValueMax / optionsettings.Max, 0, 0.99), 0, 0.99) - size, 0, 1),
          1
        ),
      })
    end
...
function mainapi:CreateCategory(categorysettings)
  ...
  function categoryapi:CreateModule(modulesettings)
    ...
    dotsbutton.MouseButton1Click:Connect(function()
      ...
      dotsbutton.BackgroundColor3 = modulechildren.Visible and color.Dark(children.BackgroundColor3, 0.05)
...
function mainapi:CreateLegit(categorysettings)
  ...
  function categoryapi:CreateModule(modulesettings)
    ...
    dotsbutton.MouseButton1Click:Connect(function()
      ...
      dotsbutton.BackgroundColor3 = settingschildren.Visible and color.Dark(children.BackgroundColor3, 0.05)
        or color.Light(uipallet.Main, 0.02)
    end)
    dotsbutton.MouseButton2Click:Connect(function()
      settingschildren.Visible = not settingschildren.Visible
...
function mainapi:Save(newprofile)
  ...
  for i, v in self.Modules do
    savedata.Modules[i] = {
      Enabled = v.Enabled,
      Bind = typeof(v.Bind) == 'Instance'
          and { Mobile = true, X = v.Bind.Position.X.Offset, Y = v.Bind.Position.Y.Offset }
        or v.Bind,
...
function mainapi:UpdateModuleColor(button, hue, sat, val, default, rainbowcheck)
  if button.Enabled then
    button.Object.BackgroundColor3 = rainbowcheck and Color3.fromHSV(mainapi:Color((hue - (button.Index * 0.025)) % 1))
      or Color3.fromHSV(hue, sat, val)
...
function mainapi:UpdateGUI(hue, sat, val, default)
  ...
  if textgui.Button.Enabled then
    VapeLogoGradient.Color = ColorSequence.new {
      ...
      ColorSequenceKeypoint.new(
        1,
        mainapi.GUIColor.Rainbow and Color3.fromHSV(mainapi:Color((hue - 0.075) % 1)) or Color3.fromHSV(hue, sat, val)
      ),
    }
    VapeLogoGradient2.Color = mainapi.GUIColor.Rainbow and VapeLogoGradient.Color
    ...
    for i, v in VapeLabels do
      v.Text.TextColor3 = customcolor
        or (
          mainapi.GUIColor.Rainbow and Color3.fromHSV(mainapi:Color((hue - ((i + 2) * 0.025)) % 1))
  ...
  for i, v in mainapi.Categories do
    ...
    if v.Type == 'CategoryList' then
      ...
      if v.Selected then
        v.Selected.BKG.BackgroundColor3 = rainbowcheck and Color3.fromHSV(mainapi:Color(hue % 1))
          or Color3.fromHSV(hue, sat, val)
