...
run(function()
        ...
        for i, inst in code do
          ...
          elseif inst.opcode == 20 then -- NAMECALL
            ...
            if kv == 'sub' or kv == 'reverse' then
              ...
              if kv == 'reverse' and not arg1 then
                arg1 = 'a'
              end
  ...
  if not jb.VehicleController.toggleLocalLocked or not jb.VehicleController.NitroShopVisible then
    repeat
      task.wait()
    until (jb.VehicleController.toggleLocalLocked and jb.VehicleController.NitroShopVisible) or LunarVape.Loaded == nil
  ...
  local function fireHook(self, id, ...)
    ...
    if InfNitro.Enabled and rem == 'UseNitro' then
      return
    end
    if LazerGodmode.Enabled and rem == 'SelfDamage' then
      return
    end
      ...
      if callback then
        ...
        jb.GunController.TransformLocalMousePosition = function(self, pos)
          ...
          if ent then
            ...
            if
              item
              and ((self.Tip.CFrame.Position - ent.RootPart.Position).Magnitude / (item.Config.BulletSpeed or 1000))
                < item.BulletEmitter.LifeSpan
            then
              ProjectileRaycast.FilterDescendantsInstances = { gameCamera, ent.Character }
              ProjectileRaycast.CollisionGroup = ent.RootPart.CollisionGroup
              local calc = prediction.SolveTrajectory(
                self.Tip.CFrame.Position,
                item.Config.BulletSpeed or 1000,
                math.abs(item.BulletEmitter.GravityVector.Y),
                ent.RootPart.Position,
                Instant.Enabled and Vector3.zero or ent.RootPart.Velocity,
                workspace.Gravity,
                ent.HipHeight,
                nil,
                ProjectileRaycast
        ...
        repeat
          ...
          if item and item.__ClassName == 'Handcuffs' then
            ...
            for _, ent in plrs do
              if not AutoArrest.Enabled then
                break
              end
