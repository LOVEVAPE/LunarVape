...
local drawingactor =
  loadstring(downloadFile 'Lunar Vape/Libraries/Drawing Library.lua', 'Lunar Vape/Libraries/Drawing Library.lua')()
    ...
    for _, v in gc do
      ...
      elseif type(v) == 'function' and islclosure(v) then
        ...
        if name == 'spawn_bullet' and debug.getinfo(v).nups > 11 then
          ...
          frontlines.ShootRay = typeof(debug.getupvalue(v, 6)) == 'RaycastParams' and debug.getupvalue(v, 6)
      ...
      if callback then
        repeat
          ...
          if inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2) then
            local origin = entitylib.isAlive and frontlines.Main.globals.fpv_sol_instances.camera_bone.WorldPosition
              or Vector3.zero
        ...
        repeat
          ...
          if AutoFire.Enabled then
            local ent = entitylib['Entity' .. Mode.Value] {
              ...
              Part = 'RootPart',
              Origin = entitylib.isAlive and frontlines.Main.globals.fpv_sol_instances.camera_bone.WorldPosition
                or Vector3.zero,
              Players = Target.Players.Enabled,
              NPCs = Target.NPCs.Enabled,
            }
    ...
    Function = function(callback)
      if callback then
        repeat
          ...
          if
            not (states.hold_ads or (time() - statetimes.trigger) < 0.2 or (time() - statetimes.press_crouch) < 0.4)
          then
          ...
          if suc then
            ...
            if #plrs > 0 then
              ...
              for i, v in plrs do
                ...
                table.insert(
                  attacked,
                  { Entity = v, Check = delta.Magnitude > AttackRange.Value and BoxSwingColor or BoxAttackColor }
                )
        ...
        repeat
          if entitylib.isAlive and pickupdelay < tick() then
            for i, v in frontlines.Main.globals.equipment_drop_ids do
              ...
              if
                obj
                and (obj.model.PrimaryPart.Position - entitylib.character.RootPart.Position).Magnitude < Range.Value
              then
